...
- Implemented the "Saved Strategies Vault". Replaced automatic saving with a manual "Save Strategy" button. Added the `SavedStrategy` schema/model in the backend with corresponding API endpoints. Displayed saved strategies in an interactive grid in the dashboard.