...
@router.post("/me/openwa/connect")
async def connect_openwa(
    user: User = Depends(require_client),
):
    """Start an OpenWA session for the client using the internal Docker URL"""
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            await client.post(f"{openwa_url}/api/sessions", headers=headers, json={"name": user.id, "sessionId": user.id})
        except Exception:
            pass
...
async def status_openwa(
    ...
    """Check OpenWA session status and fetch QR if ready"""
    openwa_url = os.getenv("OPENWA_API_URL", "http://openwa:2785")
    openwa_key = os.getenv("OPENWA_API_KEY", "")
    ...
    async with httpx.AsyncClient() as client:
        try:
            status_res = await client.get(f"{openwa_url}/api/sessions/{user.id}", headers=headers)
            if status_res.status_code == 200:
                data = status_res.json()
                state = data.get("status", "").lower()
                
                if "ready" in state or "connected" in state or "authenticated" in state:
                    return {"status": "connected"}
                
                if "qr" in state:
                    # Fetch QR Image
                    qr_res = await client.get(f"{openwa_url}/api/sessions/{user.id}/qr", headers=headers)
                    if qr_res.status_code == 200:
                        import base64
                        qr_b64 = base64.b64encode(qr_res.content).decode("utf-8")
                        return {"status": "qr_ready", "qr": f"data:image/png;base64,{qr_b64}"}
                
                return {"status": "connecting" if state in ["initializing", "authenticating", "starting", "created"] else "disconnected"}
            else:
                return {"status": "disconnected"}
        except Exception:
             return {"status": "disconnected"}

@router.post("/me/openwa/disconnect")
async def disconnect_openwa(
    user: User = Depends(require_client),
):
    """Stop and delete the OpenWA session"""
    openwa_url = os.getenv("OPENWA_API_URL", "http://openwa:2785")
    openwa_key = os.getenv("OPENWA_API_KEY", "")
    headers = {"x-api-key": openwa_key}
    ...
    async with httpx.AsyncClient() as client:
        try:
            await client.post(f"{openwa_url}/api/sessions/{user.id}/stop", headers=headers)
            await client.delete(f"{openwa_url}/api/sessions/{user.id}", headers=headers)
        except Exception:
            pass
        return {"status": "disconnected"}
